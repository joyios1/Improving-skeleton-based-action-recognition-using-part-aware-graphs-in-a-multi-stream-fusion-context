...
def split_train_val(train_indices, train_labels, method='sklearn', ratio=0.15):
    """
    ...
    """
    if method == 'sklearn':
        return train_test_split(train_indices, test_size=ratio, random_state=10000, stratify=train_labels)
    else:
...
def split_dataset(skes_joints, label, performer, camera, evaluation, save_path):
    ...
    train_indices, val_indices = split_train_val(train_indices, label[train_indices], method=m)
